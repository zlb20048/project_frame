 ...
 * 
 */
public class ProductListAction extends BaseAction implements HttpListener {
	/**
	 * 标识
	 */
	private final static String TAG = ProductListAction.class.getSimpleName();
	 ...
	 *            dataListener
	 */
	public ProductListAction(Handler handler) {
		...
		try {
			...
			if (length > 0) {
				for (int i = 0; i < length; i++) {
					...
					map.put("headurl", json1.get("headurl"));
					map.put("aurl", json1.get("aurl"));
					map.put("desc", json1.get("desc"));
					Log.v(TAG, json1.getString("title"));
					Log.v(TAG, json1.getString("bid"));
					Log.v(TAG, json1.getString("aurl"));
					list.add(map);
				}
